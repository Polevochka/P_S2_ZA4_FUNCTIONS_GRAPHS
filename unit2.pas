...
begin
  ...
  Form2.Caption:= 'График  y=' + Form1.ComboBox1.Items[Form1.ComboBox1.ItemIndex];
  ...
  // 6) Нанесение координатной сетки оцифровка осей
  ...
  // Защита - добавить точки на график
  ...
  {Рисуем через каждые 5 значений x точку на грфике функции}
  {
  Image1.Canvas.Pen.Color:= clRed; // Пусть он будет красным
  Image1.Canvas.Pen.Width:=1;
  i:= 0;
  while i <= high(GraficPoints) do
  begin
    // Получаем точку которую надо поставить
    p:= GraficPoints[i];
    // и ставим её
    Image1.Canvas.Ellipse(p.x-3, p.y-3, p.x+3, p.y+3);
    i+= 5; // переходим к следующей точке через пять едениц
  end;
  }

  {Точки в НУЛЯХ функции}
  {
  Image1.Canvas.Pen.Color:= clRed;
  Image1.Canvas.Pen.Width:=1;
  // Нули графика - это пересечение графика с линией y=0
  // y=0 в графических координатах будет
  Yg:= Yn + round(my*(Ymax-0));

  // Находим пересечения графика с этой прямой
  CrossingsY(GraficPoints, CrossPoints, Yg);
  // последовательно перебираем полученные точки
  ...
  {Точки в ЭКСТРЕМУМАХ функции}
  {
  Image1.Canvas.Pen.Color:= clRed;
  Image1.Canvas.Pen.Width:=1;
  // Экстремумы - это минимумы и максимумы графика

  // МАКСИМУМЫ находятся на линии y = Yk
  // Прсото найдём пересечения графика с этой линией
  CrossingsY(GraficPoints, CrossPoints, Yk);
  // последовательно перебираем полученные точки
  for j:=0 to high(CrossPoints) do
  begin
    p:= CrossPoints[j];
  ...
  // МИНИМУМЫ находятся на линии y = Yn
  // Прсото найдём пересечения графика с этой линией
  CrossingsY(GraficPoints, CrossPoints, Yn);
  // последовательно перебираем полученные точки
  for j:=0 to high(CrossPoints) do
  begin
    p:= CrossPoints[j];
